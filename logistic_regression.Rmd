...
outcome variable (or column) from one or more predictor variables (or
columns). This contrasts with using a binary variable *as* a predictor variable,
as we've seen on the other textbook pages.

As we've seen, a binary categorical variable is one where an observation can fall
...
of "good" and "poor", for the reasons we've seen previously.   
...
Why not use the same linear regression technique for the present data? After all, our *outcome variable* (`appetite_d`) and our *predictor variable* (`hemoglobin`) here are both sequences of values, with one value for each observation (row) in the dataset. The linear model here is shown below, using the first ten values of each vector (to stop the printout being unwieldy)!:
...
jupyprint(f"${arraytex(np.atleast_2d(appetite_d[:10]).T)} = b * {arraytex(np.atleast_2d(hemoglobin[:10]).T)} + c +" +" \\vec{\\varepsilon}$")
```

Let's compare the values in the vectors above to the first 10 values of each variable in the `hgb_app` dataframe, just to
assure ourselves they are the same:

```{python}
# for comparison with the vectors above
hgb_app[['appetite_dummy', 'hemoglobin']].iloc[:10]
```

This looks like something our linear regression machinery will be able to work with.

In contrast to the vector printout above, we actually have many more than 10 observational units:
...
$
\text{predicted_scores} =b * \text{predictor_variable} + c
$
...
$
\text{prediction_errors} = \text{actual_scores - predicted_scores}
$
...
```
predicted_appetite_d = lin_reg_slope * hemoglobin + lin_reg_intercept
```
...
The problem here is that linear regression can produce predicted values ($\hat{y}$) ranging from negative infinity to positive infinity. One way of saying this is that the predictions from linear regression are unonstrained or unbounded.
...
For the moment, let's not worry about how we came by this line, we will come
...
The sigmoid curve is exploiting the fact that probabilities range between 0 and 1,
and that the potential values that our outcome variable can take are 0 or 1. This
means we can meaningfully show the prediction line (showing the predicted 
probabilities) and the outcome variable scores (0 or 1) on the same plot.
...
Fortunately, our mathematician friends can assure us that we can in factgo from our sigmoid curve (ranging
...
(Do not worry if these steps don't make sense yet)!
...
**This has the important consequence that** we can get `minimize` to find us a straight line that
fits the data well *on the log odds scale*, then we can use our converstion to go back to
the sigmoid curve (on the original scale of the data, where the outcome variable ranges
between 0 and 1).

The next few sections go into more detail on the odds and logarithm steps.
...
$\text{probability} = \frac{\text{number of things of interest}}{\text{total number of things}}$

We can think of this same information as an *odds*.

We often express probabilities as odds.  For example, we might say that
...
The odds is the number of times we expect to see the event of interest
(e.g. rain) for every time we expect to see the event of no interest (not
rain):

$\text{odds} = \frac{\text{number of things of interest}}{\text{number of things NOT of interest}}$

This is just a way of saying a probability in a different way, and we can convert easily between probabilities and odds.

To convert from a probability to an odds, we remember that the odds is the number of times we expect to see the event of interest for every time we expect to see the event of no interest.   This is the probability (proportion) for the event of interest, divided by the probability (proportion) of the event of no interest.  Say the probability p is some value (it could be any value):
...
Notice the odds is greater than one, because the probability of an event of interest (`p`) was greater than 0.5. 

**Odds of greater than one means the event of interest is more likely to
happen than the alternative.**  

**Odds of less than one means that the probability of an event of interest was less
...
As the probability of an event of interest gets close to 1, the odds gets very large.

```{python}
p_approaching_1 = 0.9999999999999999
p_approaching_1 / (1 - p_approaching_1)
```

We can also convert from odds to probabilities.  Remember the odds is
...
the proportion of all events that are events of interest.  

We can read an `odds` of - say - 2.5 as "the chances are 2.5 to 1".  To get the
...
p_from_odds = odds / (odds + 1)
p_from_odds
```

Summary - convert probabilities to odds with:
...
Convert odds to probabilities with:
...
values to odds for our appetite predictions, we get the following:
...
Notice that the odds we found vary from very close to 0 (for p value
...
print('Logit, then inverse_logit returns the original data', back_again)
```
...
Remember, these are the predicted probabilities of a 1 label (`appetite_d == 1`).  We rename to remind ourselves:
...
The `predicted_prob_of_1` value is the probability that we will see a label of 1 for this
...
```{python}
pp_of_last_value = predicted_prob_of_1.iloc[-1]
pp_of_last_value
```
...
```{python}
pp_of_first_value = 1 - predicted_prob_of_1[0]
pp_of_first_value
```

We can therefore calculate the predicted probability of each 0 or 1 label in the
data frame like this:

```{python}
# Predicted probability of 0.
pp_of_0 = 1 - predicted_prob_of_1

# The next line sets all the label == 1 values correctly, but
# the label == 0 values are now incorrect.
pp_of_actual_label = predicted_prob_of_1

# Set the label == 0 values correctly.
poor_appetite = appetite_d == 0

pp_of_actual_label[poor_appetite] = pp_of_0[poor_appetite]

# Put this into the data frame for display
hgb_predicted['pp_of_actual_label'] = pp_of_actual_label
hgb_predicted
```

There's a fancy short-cut to the several lines above, that relies on the fact
that multiplying by 0 gives 0.  We can form the `pp_of_actual_label` column by first
multiplying the 0 or 1 label column values by the values in `predicted_prob_of_1`.  This
correctly sets the `pp_of_actual_label` values for labels of 1, and leaves the
...
pos_ps = appetite_d * predicted_prob_of_1
...
hgb_predicted['pp_of_actual_label_fancy'] = final
hgb_predicted
```
...
When the `pp_of_actual_label` values are near 1, this means the result was close to
...
`pp_of_actual_label`.  When the probabilities in `pp_of_actual_label` are all fairly near 1,
...
likelihood = np.prod(pp_of_actual_label)
likelihood
```
...
tend to be a reasonable number of small p values in `pp_of_actual_labels`.  The
...
def simple_ml_logit_cost(intercept_and_slope, x, y):
    ...
    pp_of_actual_labels = y * predicted_prob_of_1 + (1 - y) * (1 - predicted_prob_of_1)
    likelihood = np.prod(pp_of_actual_labels)
...
predicted_ML = inverse_logit(inter_ML + slope_ML * hemoglobin)
```
...
```{python}
print('Likelihood with product of array', np.prod(pp_of_actual_label))
# The log-add-unlog version.
logs_of_pp = np.log(pp_of_actual_label)
...
def mll_logit_cost(intercept_and_slope, x, y):
    ...
    log_likelihood = np.sum(np.log(pp_of_actual_labels))
    # Ask minimize to find maximum by adding minus sign.
...
predicted_MLL = inverse_logit(inter_MLL + slope_MLL * hemoglobin)
```
...
* Consider the expression `np.exp(y)` in the `inverse_logit` expression above.
...
* Consider the expression `np.sum(np.log(pp_of_actual_labels))` in the
  ...
  `np.sum(np.log10(pp_of_actual_labels))` (log to the base 10).  What would happen to:
