...
Imagine there are 100 people standing in a room. 80 of them are civil servants. If we picked a person at random, the probability they
will be a civil servant is 20/100. By contrast, the odds is the number of times we expect to see the event of interest
...
In the civil servant example, the *odds* of being a civil servant, if we pick a person at random, are 80/20. 
...
Notice the odds is less than one, because the probability of an event of interest (`p`) was less than 0.5.   
...
As the probability of an event of interest gets close to 1, the odds gets very large - this means the event of interest is many many times more likely than events of no interest:
...
Summary - we can convert probabilities to odds with:
...
values to odds, we get the following:
...
Notice that the odds we found vary very close to 0 (for probability
...
Remember, these are the predicted probabilities of a 1 label (`appetite_d == 1`).  Let's rename to remind ourselves:
...
observation.   The `appetite_d` score was, in fact, 1.  

Therefore the **predicted probability that we will see the actual value is just the corresponding
...
that multiplying by 0 gives 0.  We can form the `predicted_prob_of_actual_score` column by first
...
hgb_predicted['pp_of_actual_score_fancy'] = final
hgb_predicted
```
...
```{python}
logistic_reg_1 = minimize(simple_ml_logit_cost,  # Cost function
...
```{python}
inter_logistic_reg_1, slope_logistic_reg_1 = logistic_reg_1.x
predicted_logistic_reg_1 = inverse_logit(inter_logistic_reg_1 + slope_logistic_reg_1 * hemoglobin)
```

```{python tags=c("remove-input")}
plot_hgb_app()
plt.scatter(hemoglobin, predicted_logistic_reg_1, c='gold', label='logistic_reg_1 prediction')
plt.legend();
```
...
```{python}
logistic_reg_2 = minimize(mll_logit_cost,  # Cost function
                  [-7, 0.8],  # Guessed intercept and slope
                  args=(hemoglobin, appetite_d),  # x and y values
                  )
# Show the result.
