...
challenging if you a new to this, so please do not worry if you do not
...
def ss_any_line(b_and_c, x_vector, y_vector):
    """ Sum of squares error for intercept, slope `b_and_c`
    """
...
This essentially fits the model using a cost function this is internal to
...
- We write a cost function (`ss_any_line`) which takes a value of $b$ and $c$
  ...
  vectors. It does this by generating fitted values using $b$ and $c$,
...
The graph below shows the best fitting line for the model `prestige` = $b$
* `income + c + `  $\vec{\varepsilon}$
...
The fitted values from each separate linear regression are shown in the output of the cell below. You'll notice that the two models give different fitted values. You can think of each of these regressions as modelling the linear relationship between each predictor and the outcome variable if we **ignore** the other predictor.
...
You've already seen that it is possible to include multiple predictors in a linear regression model on the [multiple regression page](https://lisds.github.io/textbook/classification/Multiple_Regression.html)). 

In fact, the original model Duncan fit included both `education` and `income` as predictor variables - (and as mentioned previously, the parameter estimates were calculated with a mechanical calculator, a labour which we will not subject you to!).
...
Remember that our predictor vector `education` can also be referred to by it's mathematical name $\vec{x}$? We might want to use different symbols to refer to our second slope and second predictor, so we could write something like: 
...
* $x_{1_{15}}$ is the *fiftheenth* value from $\vec{x_1}$
...
* $x_{2_{15}}$ is the *fiftheenth* value from $\vec{x_2}$
...
$c$ is the intercept (again, including this let's us fit our model more flexibly, just as when we use a single predictor)
...
<!-- #region -->
- we then pass our cost function to `minimize` and it tries multipe $b$ and $c$ pairs until it finds the pair that gives the lowest value of the cost function (e.g. the lowest sum of the squared error), out of all possible $b$ and $c$ pairs
...
You'll not that despite there now being two predictors and two slopes, *we still only get one set of fitted values*. That is what makes this a *multiple regression model* rather than two separate linear regressions. We performed two separate linear regressions above, and they produced *two* separate vectors of fitted values, and *two* separate error vectors.
...
This is exactly the same process as for a single predictor model - *only now the fitted values are generated using informaton from two predictor variables*.
...
```{python}
# our arbitrary guesses for the slopes and intercept
...
So with our guessed parameter estimates, the model with two predictors could be written as follows:
...
Just as the formula for the model, this can be modified for two predictors:
...
The code cell below let's us make different guesses about the parameter estimates (the slopes and intercepts $b_1, b_1, c $) and shows us the error vector and the sum of the squared error, for that particular combination.
...
The value of the sum of the squared error depends on our particular combination of parameter estimates ($b_1, b_1, c $). We can use `minimize` to try lots of combinations in order to identify the combination which gives the smallest sum of the squared error.

In order to do that, we need to modify our cost function so that it accepts two predictors, and generates fitted values which result from those two predictors, and two slopes:

```{python}
# modify our sum of squares function to accept two predictors
def ss_two_predictors(bs_and_c, x1_vector, x2_vector, y_vector): # this now takes an extra predictor vector
...
$\begin{bmatrix}{} \text{$y_{1}$} \\ \text{$y_{2}$} \\ \text{$y_{3}$} \\ \text{$y_{4}$} \\ \text{$y_{5}$} \\ \text{$y_{6}$} \\ \text{$y_{7}$} \\ \text{$y_{8}$} \\ \text{$y_{9}$} \\ \text{$y_{10}$} \\ \text{$y_{11}$} \\ \text{$y_{12}$} \\ \text{$y_{13}$} \\ \text{$y_{14}$} \\ \text{$y_{15}$} \\ \end{bmatrix} = b_1 * \begin{bmatrix}{} \text{$x_{11}$} \\ \text{$x_{12}$} \\ \text{$x_{13}$} \\ \text{$x_{14}$} \\ \text{$x_{15}$} \\ \text{$x_{16}$} \\ \text{$x_{17}$} \\ \text{$x_{18}$} \\ \text{$x_{19}$} \\ \text{$x_{110}$} \\ \text{$x_{111}$} \\ \text{$x_{112}$} \\ \text{$x_{113}$} \\ \text{$x_{114}$} \\ \text{$x_{115}$} \\ \end{bmatrix} + b_2 *  \begin{bmatrix}{} \text{$x_{21}$} \\ \text{$x_{22}$} \\ \text{$x_{23}$} \\ \text{$x_{24}$} \\ \text{$x_{25}$} \\ \text{$x_{26}$} \\ \text{$x_{27}$} \\ \text{$x_{28}$} \\ \text{$x_{29}$} \\ \text{$x_{210}$} \\ \text{$x_{211}$} \\ \text{$x_{212}$} \\ \text{$x_{213}$} \\ \text{$x_{214}$} \\ \text{$x_{215}$} \\ \end{bmatrix}  +c + \begin{bmatrix}{} \text{$\varepsilon_{1}$} \\ \text{$\varepsilon_{2}$} \\ \text{$\varepsilon_{3}$} \\ \text{$\varepsilon_{4}$} \\ \text{$\varepsilon_{5}$} \\ \text{$\varepsilon_{6}$} \\ \text{$\varepsilon_{7}$} \\ \text{$\varepsilon_{8}$} \\ \text{$\varepsilon_{9}$} \\ \text{$\varepsilon_{10}$} \\ \text{$\varepsilon_{11}$} \\ \text{$\varepsilon_{12}$} \\ \text{$\varepsilon_{13}$} \\ \text{$\varepsilon_{14}$} \\ \text{$\varepsilon_{15}$} \\ \end{bmatrix}$
...
- we pass our cost function to `minimize` and it tries multipe $b_1$, $b_2$ and $c$ combinations until it finds the combination that gives the lowest value of the cost function, out of all possible $b_1$, $b_2$ and $c$ combinations
