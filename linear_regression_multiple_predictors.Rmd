...
a linear regression model on the [multiple regression
...
of parameter estimates ($b_1, b_1, c $). We can use `minimize` to try lots of
combinations in order to identify the combination which gives the smallest sum
of the squared error.

In order to do that, we need to modify our cost function so that it accepts two
predictors, and generates fitted values which result from those two predictors,
and two slopes:
