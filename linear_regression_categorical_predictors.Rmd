...
The *lab book principle* states: to work out what type of variable something is, think about how you would record each observation in a lab book, if you were collecting the raw data yourself. If you would write a number when recording the raw data for the variable, then it is a numerical variable. If you would write a qualitative category label when recording the raw data for the variable, then the variable is a categorical variable.

So when recording the number of children each person on the street has we would write a number (0, 1, 2, 3 and so on) in the lab book.
...
```{python}
jupyprint('### `prestige ~ income` (ignoring `type`)')
plot_model_3D(x1_slope = 0, x2_slope = income_single_reg.slope , c = income_single_reg.intercept,
             show_errors = False)
```
...
```{python}
jupyprint('### `prestige ~ type` (ignoring `income`)')
plot_model_3D(x1_slope = type_single_reg.slope, x2_slope = 0 , c = type_single_reg.intercept,
             show_errors = False)
```

```{python}
jupyprint('### `prestige ~ type + income`')
make_3d_scatter(x1 = type_dummy, x2 = income, y = prestige)
```
...
```{python}
# here to check parameters, might remove
import statsmodels.formula.api as smf

import warnings  
warnings.filterwarnings("ignore", message="kurtosistest only valid for n>=20", category=UserWarning)

for model_spec in ["prestige ~ income", "prestige ~ type_dummy", "prestige ~ type_dummy + income"]:
    display(smf.ols(model_spec, data = df).fit().summary())
```
