...
Now, let's we imagine the graphs as physical objects, where you could stand on one axis and look at the graph. The image below shows some humanlike figures added to the graph, with a blue line indicating which direction they are looking:
...
Here is what the person on the rightmost graph (shown all in black) would see:
...
The three graphs are shown again below, for convenience. If you look at the rightmost graph, you'll see that the red regression plane, with respect to each axis, has been **adjusted** when we fit the plane taking into account both variables, relative to the other two graphs, where for each we ignore the other predictor variable when fitting the plane.

![](images/stat_adjust.png)

If we compare the `type` slope on the middle and rightmost graph, you can see that it is smaller on the right most graph (the slope is called $b_1$ on both graphs).
...
- for two observational units (occupations) which had equal `income` but one was classified as "professional" (`prof`) and the other as "white collar" (`wc`), then based on the relationships in our dataset we would predict they would differ in `prestige` by 29.98 percentage points (in favour of `prof` jobs)".
